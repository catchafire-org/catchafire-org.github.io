...
function normalizeStyle(value) {
  if (isArray$1(value)) {
    ...
    for (let i = 0; i < value.length; i++) {
      const item = value[i];
      const normalized = isString$1(item) ? parseStringStyle$1(item) : normalizeStyle(item);
      if (normalized) {
...
function applyOptions(instance) {
  ...
  if (computedOptions) {
    for (const key in computedOptions) {
      ...
      const get2 = isFunction(opt) ? opt.bind(publicThis, publicThis) : isFunction(opt.get) ? opt.get.bind(publicThis, publicThis) : NOOP$1;
      const set2 = !isFunction(opt) && isFunction(opt.set) ? opt.set.bind(publicThis) : NOOP$1;
      const c = computed({
        get: get2,
        set: set2
      });
    ...
    if (forcePatchValue || patchFlag !== -1) {
      ...
      if (props) {
        if (forcePatchValue || !optimized || patchFlag & (16 | 32)) {
          for (const key in props) {
            if (forcePatchValue && key.endsWith("value") || isOn$1(key) && !isReservedProp$1(key)) {
              patchProp2(
                el,
                key,
                null,
